...
class OtpInput extends Component {
  ...
  getPlaceholderValue = () => {
    const {placeholder, numInputs = 4} = this.props;
  ...
  handleOtpChange = (otp) => {
    const {onChange, numInputs} = this.props;
    const otpValue = otp.join('');
    this.setState({isDisabled: otp.length !== numInputs});
  ...
  render() {
    ...
    return (
      <Modal className="otp-model" isOpen={open} centered>
