...
import { orderType, objectToFormData } from '../../helper'
import {
  IndividualMealsTable,
  MealPlansTable
} from "./orderTable";
          ...
          <div>
            {Object.keys(sort).length ? <span style={styles.badge} onClick={() => setSort({})}>Clear all</span> : null}
            {
              type === orderType.INDIVIDUAL_MEAL ?
                <IndividualMealsTable meals={allMeals} sort={sort} onSorting={handleSorting} handleDelivered={handleDelivered} /> :
                <MealPlansTable meals={allMeals} sort={sort} onSorting={handleSorting} handleDelivered={handleDelivered} />
            }
          </div>
        }
      </Box>
